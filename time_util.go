...
// 返回本月1号， 属于今年第几天
func GetYearDayOfCurrentMonthFirstDay(now time.Time) int {
	monthTime := time.Date(now.Year(), now.Month(), 1, 0, 0, 0, 0, now.Location())
	now.Month()
	return monthTime.YearDay()
}

// 返回本月最后一天， 属于今年第几天
func GetYearDayOfCurrentMonthLastDay(now time.Time) int {
