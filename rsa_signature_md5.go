...
// 签名
func RSASignWithMD5(privateKey string, content string) (sign string) { // pack8
	hash := md5.New()
...
func RSAPack1SignWithMD5(privateKey string, content string) (sign string) {
	hash := md5.New()
	io.WriteString(hash, string(content))
	hashed := hash.Sum(nil)
	...
	signature, err := rsa.SignPKCS1v15(rand.Reader, pk, crypto.MD5, hashed)
	if err != nil {
		fmt.Println("rsa_sign_error", err)
		return
	}
